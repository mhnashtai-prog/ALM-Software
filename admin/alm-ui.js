...
Object.keys(byLevel).forEach(key => {
  ...
  rows.forEach(row => {
   ...
   const ledgerStyle = `font-size:7px;font-weight:700;padding:2px 6px;border:1px solid;cursor:pointer;transition:all .12s;white-space:nowrap;font-family:var(--mono);border-radius:2px;`;
    ...
    rowsHTML += `<div class="barchart-row" onclick="ovDrillToFormation('${key}')"><div class="barchart-row-label" style="display:flex;align-items:center;justify-content:flex-end;gap:4px"><div style="width:7px;height:7px;border-radius:50%;background:${healthBg};flex-shrink:0"></div>${label}</div><div class="barchart-row-track" style="max-width:62%">${placed > 0 ? `<div class="barchart-segment" style="width:${pPlaced}%;background:var(--green)">${placed >= 6 ? placed : ''}</div>` : ''}${waiting > 0 ? `<div class="barchart-segment" style="width:${pWait}%;background:var(--amber)">${waiting}</div>` : ''}${noReq > 0 ? `<div class="barchart-segment" style="width:${pNoReq}%;background:var(--red)">${noReq >= 6 ? noReq : ''}</div>` : ''}${pEmpty > 0 ? `<div class="barchart-segment" style="width:${pEmpty}%;background:rgba(255,255,255,.04)"></div>` : ''}</div><div style="display:flex;align-items:center;gap:4px;flex-shrink:0;margin-left:8px"><span style="font-size:8px;font-weight:700;color:var(--t3);font-family:var(--mono);min-width:22px;text-align:right">${total}</span><span style="font-size:7px;color:var(--t4);font-family:var(--mono);min-width:28px">${placedPct}%</span>${icons}</div></div>`;
  });
  rowsHTML += `</div>`;
