...
async function openDossier(ref) {
  ...
  // U-02: section previews
  const recentYear = hst[0];
  const histPreview = recentYear
    ? `${recentYear.academic_year || '—'} · ${recentYear.outcome || 'em curso'}`
    : hst.length ? `${hst.length} ano${hst.length > 1 ? 's' : ''}` : '—';
  const slots = parseSlotsForRuler(req);
  const ttPreview = slots.length
    ? slots.slice(0, 2).map(s => `${s.day} ${s.startLabel}`).join(' · ') + (slots.length > 2 ? ` +${slots.length - 2}` : '')
    : '—';
  const notesPreview = enrol?.notes
    ? enrol.notes.slice(0, 40) + (enrol.notes.length > 40 ? '…' : '')
    : '';
  ...
  // U-05: removed duplicate slotTags row — ruler only in dsLoadTimetable
  ...
  // U-02: Inscrição open by default; other sections show previews
  ...
  // Wire section expand/collapse + lazy timetable load
